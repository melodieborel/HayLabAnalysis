...
directory = "//10.69.168.1/crnldata/waking/audrey_hay/L1imaging/AnalysedMarch2023/Gaelle/Baseline_recording_ABmodified/AB_analysis/Analysis_VigStates_2024-06-13_17_57_05_192411_noN2_Signpref/"
...
for NrSubtype in NrSubtypeList:
    ...
    AllUnits = combined_df['Unit_ID'].unique()
    ...
    AresultSpikeActivity_perUnit = combined_df.pivot_table(index='Unit_ID', columns='Substate', values='DeconvSpikeMeanActivity', aggfunc='mean')    
    AresultSpikeActivity_perUnit = AresultSpikeActivity_perUnit[desired_order]
    ...
    REMprefUnits=index_lists['REM']
    WakeprefUnits=index_lists['Wake']
    AllUnits = combined_df['Unit_ID'].unique()

    # Save the List of significant Unit more active in one vigilance state
    filenameOut = f'{folder_to_save}/{NrSubtype}_AverageFiringPreference.xlsx'
    writer = pd.ExcelWriter(filenameOut)
    ...
    for listnb, listI  in enumerate(List_SignFiringPreference):
        ...
        filtered_df = combined_df[combined_df['Unit_ID'].isin(listI)]
        ...
        if len(filtered_df)>0:
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_Rsquared_CaCorrCoeff_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_Rsquared_SpCorrCoeff_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            # Save CorrCoeff_calcium_perUnit
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_CaCorrCoeff_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            CorrCoeff_calcium_perUnit.to_excel(writer)
            writer.close()
            ...
            # Save CorrCoeff_spike_perUnit 
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_SpCorrCoeff_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            CorrCoeff_spike_perUnit.to_excel(writer)
            writer.close()
            ...
            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_AUC_perUnitAB_N.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            resultNormalizedAUC_calcium_perUnit.to_excel(writer)
            writer.close()

            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_DiffVigSt_AUC_perUnitAB_N.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_AUC_perMouseAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_Spike_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            resultSpikeActivity_perUnit.to_excel(writer)
            writer.close()

            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_DiffVigSt_Spike_perUnitAB_N.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_Spike_perMouseAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_ActivityVigSt_proportions.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            ...
            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_DeconvSpike_perUnitAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            resultSpikeActivity_perUnit.to_excel(writer)
            writer.close()

            # Save  df
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_DiffVigSt_DeconvSpike_perUnitAB_N.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            Diff_resultSpikeActivity_perUnit.to_excel(writer)
            writer.close()
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_VigSt_DeconvSpike_perMouseAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            resultSpikeActivity_perMouse.to_excel(writer)
            writer.close()

            proportions = resultSpikeActivity_perUnit['Activated_by'].value_counts(normalize=True)*100
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_ActivityVigSt_DeconvSpike_proportions.xlsx'
            writer = pd.ExcelWriter(filenameOut)
            proportions.to_excel(writer)
            writer.close()
            ...
            filenameOut = f'{folder_to_save}/{List_name}/{NrSubtype}_ProportionVigStatesAB.xlsx'
            writer = pd.ExcelWriter(filenameOut)
