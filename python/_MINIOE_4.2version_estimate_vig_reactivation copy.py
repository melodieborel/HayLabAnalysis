...
destination_folder= f"//10.69.168.1/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/4_Reactivation_{FolderNameSave}_{AnalysisID}" if local else f"/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/4_Reactivation_{FolderNameSave}_{AnalysisID}"
...
data = {}
counter=0
CellAssembly_GlobalResults= pd.DataFrame(data, columns=['Mice','NeuronType','Session', 'Session_Date', 'Session_Time', 'Day', 'Trial', 'Assembly_ID', 'Assembly_size', 
                                                        'Cells_in_Assembly','ExpeType', 'Drug', 'Substate', 'SubstateDuration', 'Session_ID', 
                                                        'Avg_ReactStr', 'EventFreq', 'EventTime' ])

filenameOut =  f'//10.69.168.1/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/_CellAssemblies_2026-05-06_10_53_22_Ca_activity_20bins_mp/CellAssembly_dict.pkl' if local else f"/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/_CellAssemblies_2026-05-06_10_53_22_Ca_activity_20bins_mp/CellAssembly_dict.pkl"
with open(filenameOut, 'rb') as pickle_file:
    CellAssembly_dict = pickle.load(pickle_file)

filenameOut =  f'//10.69.168.1/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/_CellAssemblies_2026-05-06_10_53_22_Ca_activity_20bins_mp/CellAssembly_members.pkl' if local else f"/crnldata/forgetting/Théa/MiniscopeOE_analysis/Exploration_task/_CellAssemblies_2026-05-06_10_53_22_Ca_activity_20bins_mp/CellAssembly_members.pkl"
with open(filenameOut, 'rb') as pickle_file:
...
for dpath in Path(dir).glob('**/mappingsAB.pkl'):
    ...
    for minianpath in minian_folders: # for each minian folders found in this mouse 
        ...
        if 1==1: # any(p in all_expe_types for p in minianpath.parts): # have to be to the expe_types
            ...
            if len(patterns) > 0:
                if session_type not in ['SleepBefore', 'SleepAfter']: #####################
                    continue
                ...
                for ass in np.arange(np.shape(patterns)[0]):
                    ...
                    import re
                    parts = re.split(r'[\\/]+', matches[0])  # split robuste sur / et \
                    assembly_date = next((p for p in parts if re.match(r'\d{4}_\d{2}_\d{2}$', p)), None)

                    print(f"  assembly_date='{assembly_date}' vs session_date='{session_date}'")
                    if assembly_date is not None and assembly_date == session_date: # make sure the cell assembly was detected on the same day
                        ...
                        print(f"  signcells_in_template: {sum(signcells_in_template)}/{len(signcells_in_template)} cellules communes")
                        print(f"  any(signcells_in_template) = {any(signcells_in_template)}")
                        
                        if any(signcells_in_template): 
                            print(f"  >>> Assembly ACCEPTED: {assembly_ID}")
                            ...
                            for m in mapp:  
                                ...
                                CellAssembly_GlobalResults.loc[counter, 'Day'] = int(day)
                                CellAssembly_GlobalResults.loc[counter, 'Trial'] = int(trial)                   
                                ...
                                counter+=1
                                print(f"      {mapp[m]} -> counter is now {counter}")
