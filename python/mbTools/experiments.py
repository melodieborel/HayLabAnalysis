...
class experiment():
   ...
   def loadCurrentFolder(self,currentFolder):
      if currentFolder == "":
         ...
         self.expe_path = path
      elif os.path.isfile(os.path.join(currentFolder,self.parser_fn)):
         print(f'current folder {currentFolder} contains a config file')
         self.expe_path = currentFolder
         parserName = os.path.join(currentFolder,self.parser_fn)
         ...
         self.config.set('GENERAL','currentFolder', self.expe_path)
         ...
         if self.project_type == 1:
            self.interim_analysis_path = os.path.join(self.analysis_path_root, self.project_id,  self.sub_project_id, self.config['ANALYSIS']['interimpath'], self.condition_id, self.animal_id, self.recording_id)
         else:
            self.interim_analysis_path = os.path.join(self.analysis_path_root, self.project_id,  self.sub_project_id, self.config['ANALYSIS']['interimpath'], self.animal_id, self.condition_id, self.recording_id)
   ...
   def updateExpeConfigFile(self):
      ...
      configFN = os.path.join(self.interim_analysis_path,self.parser_fn)
   ...
   def analyseExpe_findData(self, fullSampling=False):
      ...
      if self.find_files_with_string(self.interim_analysis_path,  "RawDataChannelExtractedDS.npy"): # pre-analysed data
         print('********found some RawDataChannelExtractedDS.npy files********')
         matching_files = self.find_files_with_string(self.interim_analysis_path, "RawDataChannelExtractedDS.npy")
         self.data['LFP_DS'] = LFP_DS(self, matching_files)
         DSdata=True

      if self.find_files_with_string(self.interim_analysis_path,  f"{self.spindle_bn}_*.csv"): #NPX's Data
         print('********found some Spindles files********')
         matching_files = self.find_files_with_string(self.interim_analysis_path, f"{self.spindle_bn}_*.csv")
      ...
      if fullSampling or not DSdata:
         if self.find_files_with_string(self.raw_data_path,  ".bin"): #Bonsai or IgorPro
            print('********found some .bin files********')
            matching_files = self.find_files_with_string(self.raw_data_path, ".bin")
            self.data['OE_LFP'] = IntanLFP(self, matching_files, numChannels=self.num_lfp_channels)
      
         if self.find_files_with_string(self.raw_data_path,  "continuous.dat"): #OpenEphys
            print('********found some continuous.dat files********')
            matching_files = self.find_files_with_string(self.raw_data_path, "continuous.dat")
            self.data['OE_LFP'] = IntanLFP(self, matching_files, recSyst = 'OpenEphys', numChannels=self.num_lfp_channels)


      if self.find_files_with_string(self.raw_data_path,  "NP_spikes_*.raw"): #NPX's Data
         print('********found some NPX files********')
         matching_files = self.find_files_with_string(self.raw_data_path, "NP_spikes_*.raw")
   ...
   def loadAnimalInfo(self):
      ...
      if int(self.project_type) == 0:
         animalConf = os.path.join(self.analysis_path_root, self.project_id, self.sub_project_id, self.config['ANALYSIS']['interimpath'], self.condition_id, self.animal_id, animalConfBN)
      else:
         animalConf = os.path.join(self.analysis_path_root, self.project_id, self.sub_project_id, self.config['ANALYSIS']['interimpath'], self.animal_id, animalConfBN)
