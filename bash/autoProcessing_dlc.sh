...
for pathtofolder in $(find "$START_DIR" -type f -name "*.avi" -exec dirname {} \; | sort -u); do
    ...
    # if [[ "$pathtofolder" == *"My_First_WebCam"* && "$pathtofolder" == *"Cheeseboard"* ]]; then  #only process cheeseboard movies 
        ...
        #srun --mem=90G --cpus-per-task=20 python /home/aurelie.brecier/HayLabAnalysis/python/DLC_analyze_videos.py
        srun --mem=20G python /home/aurelie.brecier/HayLabAnalysis/python/DLC_analyze_videos.py
