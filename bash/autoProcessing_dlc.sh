...
for pathtofolder in $(find "$START_DIR" -type f -name "*.avi" -exec dirname {} \; | sort -u); do
    ...
    if [[ "$pathtofolder" == *"My_First_WebCam"* && "$pathtofolder" == *"/Cheeseboard/"*  &&  "$pathtofolder" == *"Habituation"* ]]; then  #only process cheeseboard movies 

        echo "Found folder: $pathtofolder"

        rm -rf /mnt/data/TheaM_minian/* #empty mnt data
        cp -r "${pathtofolder}/"* /mnt/data/TheaM_minian/ #copy crnldata to mnt data 
        
        #srun --mem=250G --cpus-per-task=40 python /home/aurelie.brecier/HayLabAnalysis/python/DLC_2_analyze_videos.py
        srun --partition=GPU --mem=20G --cpus-per-task=4 --gres=gpu:1g.20gb:1 python /home/thea.michel/HayLabAnalysis/python/DLC_2_analyze_videos.py
        ...
        cp -r /mnt/data/TheaM_minian/* "${pathtofolder}/" #copy dlc folder of mnt data to crnldata 
        rm -rf /mnt/data/TheaM_minian/* #empty mnt data
    fi
done
